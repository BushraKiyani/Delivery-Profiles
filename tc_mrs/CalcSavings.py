...
def changeDates(df_basedata_MR, df_pattern): #set all dates to pattern days
    ...
    df_basedata_MR["Kalenderwoche"] = df_basedata["Beladedatum"].dt.week
...
def simple_MR(df_basedata_MR_filter, max_cap, MR_price, KW, tag, MR_ID):
    ...
    date = datetime.strptime("{} {} {}".format(tag + 1, KW - 1, 2020), "%w %W %Y")
...
def calcMilkrun(df_basedata_MR, df_milkruns, df_pattern, df_basedata_no_MR, no_MR_weeks,MR_Knappsack):
    ...
    for KW in range(1,53+1):#range(df_basedata_MR["Kalenderwoche"].min(), df_basedata_MR["Kalenderwoche"].max()+1):
            ...
            if date.year == 2020:
                for index, row in df_milkruns_filter.iterrows():
                    ...
                    if KW not in no_MR_weeks:
                        ...
                        data_MR["Auslastung"].append(df_basedata_MR_filter["Gewicht"].sum()/row["Kapazität"])
                        ...
                        if MR_Knappsack == True:
                            df_basedata_MR_applied = knappsack_MR(df_basedata_MR_filter, row["Kapazität"],
    ...
    df_MR_collected.to_csv(
                r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Ergebnisse\Feinergebnisse\only_MR\df_only_MR_Sendungsdaten_"+instanzname+""+"_außer_KW"+str(no_MR_weeks)+"Knappsack_"+str(MR_Knappsack)+".csv",
    ...
    df_data_MR.to_csv(
                r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Ergebnisse\Auswertung\df_MR_Auslastung_"+instanzname+""+"_außer_KW"+str(no_MR_weeks)+"Knappsack_"+str(MR_Knappsack)+".csv",
                encoding="latin_1", sep=";", index=False)

    df_MR_complete.to_csv(
                r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Ergebnisse\Feinergebnisse\all\df_MR_Sendungsdaten_"+instanzname+""+"_außer_KW"+str(no_MR_weeks)+"Knappsack_"+str(MR_Knappsack)+".csv",
                encoding="latin_1", sep=";", index=False)
...
def diagrammdaten(df_data_MR, df_MR_complete, df_basedata, no_MR_weeks, MR_Knappsack):
    ...
    df_costs_compare_KW.to_csv(
        r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Ergebnisse\Auswertung\Diagrammdaten\df_MR_Einsparungen_" + instanzname + "" + "_außer_KW" + str(
...
def zusammenfassung(instanzname, no_MR_weeks, MR_Knappsack, df_AFNodes, df_pattern, df_data_MR_and_not_MR, df_MR_collected, df_milkruns, df_Var):
    ...
    df_zus.to_csv(
        r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Ergebnisse\Zusammenfassungen\df_MR_Zusammenfassung_" + instanzname + "" + "_außer_KW" + str(
            no_MR_weeks) + "Knappsack_" + str(MR_Knappsack) + ".csv",
        encoding="latin_1", sep=";", index=False)
...
if __name__ == '__main__':
    no_MR_weeks = [1,53]
    instanzname = "0.75-0.75-SZ20Multi1.23-600-1800"
    MR_Knappsack = True

    df_basedata = pd.read_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Datensatz_TK_fertig.csv", encoding="latin-1", sep=";")
    ...
    df_pattern = pd.read_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Ergebnisse\Rohergebnisse\df_patterns_TK_Instanz"+instanzname+".csv", encoding="latin-1", sep=";")
    ...
    df_milkruns = pd.read_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Ergebnisse\Rohergebnisse\df_tours_TK_Instanz"+instanzname+".csv", encoding="latin-1", sep=";",converters={'Sequenz': eval})
    print(df_milkruns.dtypes)
    df_milkruns["Sequenz_extID"] = df_milkruns.apply(lambda row : convertIDs(df_pattern,row["Sequenz"]), axis = 1)
    print(df_milkruns)
    df_milkruns.to_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Ergebnisse\Rohergebnisse\df_tours_TK_Instanz"+instanzname+".csv",encoding="latin_1", sep=";", index=False)

    df_AFNodes = pd.read_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Ergebnisse\Rohergebnisse\df_AFNodes_TK_Instanz"+instanzname+".csv", encoding="latin-1", sep=";")
    print(df_AFNodes)

    df_Var =  pd.read_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Variabiltätsauswertung\Variablitaetsauswertung.csv", encoding="latin-1", sep=";")
    print(df_Var)

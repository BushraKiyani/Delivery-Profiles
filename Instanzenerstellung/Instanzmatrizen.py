...
if __name__ == '__main__':
    df_var = pd.read_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Variabiltätsauswertung\Variablitaetsauswertung.csv", encoding="latin_1", sep=";", index_col= "ID_Empfänger")
    df_TK = pd.read_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Datensatz_TK_fertig.csv", encoding="latin_1", sep=";", index_col= "ID_Empfänger")
    ...
    #df_var filtern
    df_var = df_var[(df_var["avg_Frequenz"]>=1) & (df_var["variability_Gewicht"] <= var_gew) & (df_var["variability_Frequenz"] <= var_freq)]
    ...
    df_instance = df_instance[["ID_Empfänger","Name_Empfänger", "PLZ_Empfänger",  "lat", "lon", "avg_Gewicht", "avg_Frequenz"]]
    print(df_instance)

    # Add Depot
    df_depot = pd.DataFrame(data={"ID_Empfänger":[0],"Name_Empfänger": ["Depot"], "PLZ_Empfänger" : [27356] ,  "lat":[53.12433928] , "lon":[9.337199734], "avg_Gewicht": [0], "avg_Frequenz":[0]})
    df_instance = pd.concat([df_depot, df_instance])

    coordinates_list = df_instance.append({"ID": "0", "lat": 53.12433928, "lon": 9.337199734},
                                                     ignore_index=True)
    ...
    for index, row in df_instance.iterrows():
        coordinates_list.append(row["Koordinaten"])
    print(coordinates_list)

    # Euklinsische Distanzmatrix erstellen
    dist_eukl = create_matrix_eukl(coordinates_list)
    ...
    df_distmatrix_eukl.to_csv(
        r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Matrizen\TK_distmatrix_eukl_" + str(
    ...
    df_distmatrix = pd.DataFrame(columns= df_instance.ID_Empfänger.values, index= df_instance.ID_Empfänger.values, data=dist)
    df_durmatrix = pd.DataFrame(columns= df_instance.ID_Empfänger.values, index= df_instance.ID_Empfänger.values, data=dur)
    ...
    df_distmatrix_stacked.to_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Matrizen\TK_matrizen_"+str(var_gew)+"-"+str(var_freq)+".txt", sep=";",
                encoding="latin1", decimal=".", index = False)

    df_distmatrix.to_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Matrizen\TK_distmatrix_"+str(var_gew)+"-"+str(var_freq)+".csv", sep=";",
                encoding="latin1", decimal=".")
    df_durmatrix.to_csv(
    ...
    df_Tarifmatrix_long = pd.read_csv(
        r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Transportpreismatrix_TK_long.csv", encoding="latin-1",
        sep=";", dtype={"Gewicht": float, "Distanz": float})

    df_instance = add_freightcosts(df_instance, df_Tarifmatrix_long, "AF_Kosten")
    print(df_instance)

    df_instance.to_csv(r"C:\Users\Thomas\PycharmProjects\Masterarbeit\Resources\Version_2\Milkruns\Instanznodes\TK_Instanz_Nodes"+str(var_gew)+"-"+str(var_freq)+".csv", sep=";",
                encoding="latin1", decimal=".")

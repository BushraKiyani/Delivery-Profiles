    ...
    for index, row in df_touren_distanzen.iterrows():
        tarifmatrix_long_filtered = tarifmatrix_long[
            (tarifmatrix_long["Distanz"] <= row["Distanz"]) & (
                        tarifmatrix_long["Gewicht_kg"] <= row["Gewicht"])]
...
if __name__ == "__main__":
    df_touren_koordinaten = pd.read_csv(
        r"../00_Resources/Grunddaten/Datensatz_TK_bereinigt.csv", encoding="latin-1",
        sep=";", thousands=",", dtype={"Gewicht": float})

    dist_matrix_eukl = pd.read_csv(
        r"../00_Resources/Grunddaten/distance_matrix_eukl.csv",
    ...
    df_Tarifmatrix_long = pd.read_csv(r"../00_Resources/Grunddaten/Transportpreismatrix_TK_long.csv", encoding="latin-1", sep=";", dtype={"Gewicht_kg": float, "Distanz":float})
    ...
    df_touren_koordinaten_frachtkosten.to_csv(
        path_or_buf=r"../00_Resources/Grunddaten/Datensatz_TK_fertig.csv", sep=";",

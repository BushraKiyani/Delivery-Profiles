...
def runtcpvrp(instfile,suchzeit_heu,maxAllowedStops, dimafile=None, heuristicColumn_gen = False, lösungszeit = False):
    ...
    ### Print results
    df_pattern_res.to_csv(r"../00_Resources/Instances/Results/pattern/df_patterns_"+instfile.split("/")[-1].split(".t")[0]+ str(maxAllowedStops)+ "-"+ str(suchzeit_heu[0]) +"-"+ str(lösungszeit) +".csv",
            encoding="latin_1", sep=";", index=False)
    df_tours_res.to_csv(r"../00_Resources/Instances/Results/tours/df_tours_"+instfile.split("/")[-1].split(".t")[0]+ str(maxAllowedStops)+ "-"+ str(suchzeit_heu[0]) +"-"+ str(lösungszeit) +".csv",
            encoding="latin_1", sep=";", index=False)
    df_AFNodes_res.to_csv(
        r"../00_Resources/Instances/Results/AFnodes/df_AFNodes_" +
